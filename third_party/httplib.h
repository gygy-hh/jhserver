...
socket_t create_socket(const std::string &host, const std::string &ip, int port,
  ...
  // Numeric IPv4 (and empty host = 0.0.0.0): skip getaddrinfo.
  // Statically linked glibc cannot resolve via NSS, so listen("0.0.0.0") fails.
  {
    const char *numeric = !ip.empty() ? ip.c_str() : (!host.empty() ? host.c_str() : "0.0.0.0");
    sockaddr_in sin{};
    if (inet_pton(AF_INET, numeric, &sin.sin_addr) == 1) {
