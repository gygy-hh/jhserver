...
json handle_find_save(const json& req) {
  ...
  const int area = req.value("area", 0);
  if (acc.empty()) {
...
json handle_upload_save(const json& req) {
  ...
  meta.username = req.value("username", req.value("pName", acc));
  meta.lev = req.value("lev", 1);
  meta.save_time = std::time(nullptr);

  if (!storage::save_cloud(acc, area, save_str, meta)) {
    return api_err("save failed");
  }
  try {
    const int resolved_area = area > 0 ? area : 1;
    mail::sync_claimed_from_save(acc, resolved_area, resolved_area, save_str);
  } catch (...) {
  }
  return api_ok();
}
...
json handle_download_save(const json& req) {
  ...
  if (!save) {
    return json{{"code", 1}, {"error", "no save"}};
  }

  const int save_index = area > 0 ? area : 1;
  try {
    const auto pending = mail::pending_for_injection(acc, save_index);
    if (!pending.empty()) {
      std::string merged = save_blob::inject_mygift(*save, save_index, pending);
      std::vector<std::string> ids;
      ids.reserve(pending.size());
      for (const auto& m : pending) {
        ids.push_back(m.id);
      }
      mail::mark_in_save(acc, save_index, ids);
      *save = std::move(merged);
    }
  } catch (const std::exception& ex) {
    return json{{"code", 1}, {"msg", std::string("mail inject failed: ") + ex.what()}};
  }

  return json{{"code", 0}, {"data", json{{"save", *save}}}};
}
...
void handle_post(const httplib::Request& req, httplib::Response& res, const ServerConfig& config) {
  ...
  try {
    json body_json;
    if (ctx.action == "smsCode") {
      body_json = parse_sms_form(req.body);
      ...
      body_json = parse_encrypted_body(req.body, ctx.ver);
    } else {
      body_json = req.body.empty() ? json::object() : json::parse(req.body);
    }

    const json out = dispatch(ctx, body_json);
    res.set_content(respond(ctx, out), "application/json; charset=utf-8");
  } catch (const std::exception& ex) {
    std::cerr << "[error] " << ctx.action << ": " << ex.what() << std::endl;
    res.status = 400;
    if (crypto::is_encrypted_response_action(ctx.action)) {
      try {
